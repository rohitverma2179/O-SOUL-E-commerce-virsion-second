...
const initialProduct = {
  name: '', slug: '', price: '', stock: '', category: 'Unisex', type: 'Tees', shortDescription: '',
  bestFor: '', colors: 'Black, Olive', sizes: 'S, M, L, XL, XXL', tags: ''
};
const initialCombo = {
  title: '', headline: '', description: '', productNames: '', originalPrice: '', discountPercent: '', discountedPrice: ''
};
...
const CatalogManagement = () => {
  ...
  const fieldClass = 'w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-blue-500';
  ...
  const updateComboDiscount = (event) => {
    ...
    setCombo((current) => ({
      ...current,
      discountPercent: event.target.value === '' ? '' : discountPercent,
      discountedPrice: current.originalPrice === '' ? '' : Math.round(Number(current.originalPrice) * (100 - discountPercent) / 100)
    }));
  };
  ...
  const updateComboOriginalPrice = (event) => {
    ...
    setCombo((current) => ({
      ...current,
      originalPrice,
      discountedPrice: originalPrice === '' ? '' : Math.round(Number(originalPrice) * (100 - Number(current.discountPercent || 0)) / 100)
    }));
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">Catalog CMS</p>
      ...
      <div className="flex gap-2 rounded-xl bg-slate-200/60 p-1 w-fit">
        {['products', 'combos'].map((name) => (
          <button key={name} onClick={() => { setTab(name); setMessage(''); }} className={`rounded-lg px-5 py-2 text-sm font-semibold capitalize ${tab === name ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>{name}</button>
        ))}
      </div>

      <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-6 flex items-center gap-3"><PackagePlus className="text-blue-600" /><h2 className="text-xl font-bold">Add {tab === 'products' ? 'product' : 'combo'}</h2></div>
        {tab === 'products' ? (
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <input className={fieldClass} name="price" type="number" min="0" value={product.price} onChange={update(setProduct)} placeholder="Price" required />
            ...
            <label className={`${fieldClass} flex cursor-pointer items-center gap-3`}><Upload size={18} /><span>{productImage?.name || 'Choose product image'}</span><input className="hidden" type="file" accept="image/*" onChange={(e) => setProductImage(e.target.files[0])} required /></label>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-1.5 md:col-span-2"><span className="text-xs font-bold uppercase tracking-wider text-slate-500">Price after discount</span><input className={`${fieldClass} bg-slate-50`} name="discountedPrice" type="number" value={combo.discountedPrice} readOnly required /></label>
            ...
            <label className={`${fieldClass} flex cursor-pointer items-center gap-3 md:col-span-2`}><Upload size={18} /><span>{comboImages.length ? `${comboImages.length} image(s) selected` : 'Choose up to 5 combo images'}</span><input className="hidden" type="file" accept="image/*" multiple onChange={(e) => setComboImages([...e.target.files])} required /></label>
          </div>
        )}
        <button disabled={busy} className="mt-6 rounded-xl bg-blue-600 px-6 py-3 font-bold text-white hover:bg-blue-700 disabled:opacity-50">{busy ? 'Uploading...' : `Upload ${tab === 'products' ? 'product' : 'combo'}`}</button>
